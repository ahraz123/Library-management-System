...
WHERE 
    ...
    AND
    (CURRENT_DATE - ist.issued_date) > 750
ORDER BY 1;
...
WHERE member_id IN (SELECT DISTINCT issued_member_id   
                    FROM issued_status
                    WHERE issued_date >= CURRENT_DATE - INTERVAL 30 month
                    );
...
/*Task 19: Stored Procedure Objective: Create a stored procedure to manage the
 ...
 should function as follows: The stored procedure should take the book_id as an input
 parameter. The procedure should first check if the book is available (status = 'yes'). 
 If the book is available, it should be issued, and the status in the books table 
 should be updated to 'no'. If the book is not available (status = 'no'), the
 procedure should return an error message indicating that the book is currently 
 not available.*/
...
BEGIN
    ...
    ELSE
        SELECT CONCAT('Sorry to inform you, the book you requested is unavailable. book_isbn: ', p_issued_book_isbn) AS message;
    END IF;
END$$
